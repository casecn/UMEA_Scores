...
def collect_scores_and_round_guids(
    ...
    for season_name, season_id, in season_guid_dict.items():
        ...
        for c in competitions:
            ...
            rows = flatten_competition_results(comp_data=comp_data, season_name=season_name)
            all_rows.extend(rows)

            for r in rows:
                guid = r.get('round_guid')
            if guid:
...
def build_season_scores_csv(season_id, season_name, out_path):
    competitions = get_competitions_for_season(season_id)
    ...
    for c in competitions:
        ...
        # Collect recap URLs for this competition
        for r in rows:
            #print(f'R = {r}')
            guid = r.get("round_guid")  
            print(guid)# this is your full_recap_url
            if guid:
                unique_round_guid.add(guid)
